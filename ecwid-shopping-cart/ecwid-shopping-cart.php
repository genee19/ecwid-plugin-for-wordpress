...
if (!empty($ecwid_product_id) || !empty($ecwid_category_id)) {
...
//rewriting links for seo friendly url 
function add_rewrite_rules( $wp_rewrite ) { 
    $new_rules = array(
        "(".get_page(get_option("ecwid_store_page_id"))->post_name.")/.*[^-]?-c-(\d*)" => 
			'index.php?pagename='.$wp_rewrite->preg_index(1).'&ecwid_category_id=' . $wp_rewrite->preg_index(2), 
        "(".get_page(get_option("ecwid_store_page_id"))->post_name.")/.*[^-]?-p-(\d*)" => 
			'index.php?pagename='.$wp_rewrite->preg_index(1).'&ecwid_product_id=' . $wp_rewrite->preg_index(2));
    $wp_rewrite->rules = $new_rules + $wp_rewrite->rules; 
}
add_action('generate_rewrite_rules', 'add_rewrite_rules');
function get_query_vars( $qvars ){ 
    $qvars[] = 'ecwid_category_id'; 
    $qvars[] = 'ecwid_product_id'; 
    return $qvars; 
} 
add_filter('query_vars', 'get_query_vars' ); 
 
function test_flush_rules(){
    global $wp_rewrite; 
    $wp_rewrite->flush_rules(); 
} 
add_action('ecwid_store_activate','test_flush_rules');
...
function add_ecwid_admin_bar_node() {
    ...
    $wp_admin_bar->add_menu( array(
		'id' => 'ecwid_main',
		'title' => '<img src="'.plugins_url().'/ecwid-shopping-cart/icon.jpg" style="width: 22px;height: 22px;margin-top: 3px;"/>',
	));
	//add ecwid home page
	$wp_admin_bar->add_menu(array(
			"id" => "ecwid_home",
...
static $ecwid_cat_prod_data = null;
...
function ecwid_get_mixed_data($get_category_only = false){
	...
	if ($get_category_only){
		$params = array(
			array("alias" => "c", "action" => "category", "params" => array("id" => $ecwid_category_id))
		);
...
function ecwid_seo_title($content) {
	...
	$ecwid_product_id = intval($wp_query->query_vars['ecwid_product_id']);
	$ecwid_category_id = intval($wp_query->query_vars['ecwid_category_id']);
	if (!empty($ecwid_product_id) && !empty($ecwid_noscript_seo_catalog)) {
		$ecwid_seo_product_title = '';
		static $ecwid_cat_prod_data;
		$ecwid_cat_prod_data = ecwid_get_mixed_data();
		$ecwid_seo_product_title = $ecwid_cat_prod_data['p']['name'];//$ecwid_product['name'];
		return $ecwid_seo_product_title." | ";
	}elseif (!empty($ecwid_category_id) && !empty($ecwid_noscript_seo_catalog)) {
		$ecwid_seo_category_title = '';
		static $ecwid_cat_prod_data;
		$ecwid_category_data = ecwid_get_mixed_data(true);
		print_r($ecwid_category_data);
		$ecwid_seo_category_title = $ecwid_cat_prod_data['c']['category'];//$ecwid_product['name'];
		return $ecwid_seo_category_title." | ";
	}else {
		return $content;
	}  
}
...
function ecwid_productbrowser_shortcode() {
    ...
    $params = array(
      ...
      'ecwid_show_seo_catalog' => get_option('ecwid_noscript_seo_catalog'),
      'ecwid_mobile_catalog_link' => get_option('ecwid_mobile_catalog_link'),

      //'ecwid_enable_html_mode' => '', // deprecated
    );
...
function ecwid_sso() {
    ...
    if ($current_user->ID) {
        ...
        $sso_string = "<script> var ecwid_sso_profile='$user_data $hmac $time';</script>";
    } else { //no cur userID
        $sso_string = "<script> var ecwid_sso_profile='' </script>";
    }
	return $sso_string . "<script> Ecwid.OnAPILoaded.add(function() {
		window.Ecwid.setSignInUrls({ signInUrl: '" . get_bloginfo('wpurl') .
		"/wp-login.php', signOutUrl: '" . get_bloginfo('wpurl') .
		"/wp-login.php?action=logout&_wpnonce=" . md5(rand()) . "' });
		});</script>";
